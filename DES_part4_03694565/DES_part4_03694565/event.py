...
class CustomerArrival(SimEvent):
    ...
    def process(self):
        ...
        cus_event = CustomerArrival(self.sim, self.sim.sim_state.now + self.sim.rng.get_iat()*1000)
        ...
        if self.sim.system_state.add_packet_to_server():#add to the server
            ...
            service_event = ServiceCompletion(self.sim, self.sim.sim_state.now + self.sim.rng.get_st()*1000)
